...
class Net(nn.Module):
    def __init__(self, cfg, dataset):
        ...
        self.localmodel = SpatialAttention2d(backbone_out)
    ...
    def forward(self, batch, scale=1):
        ...
        block3 = x.detach()
        local_features, att_score = self.localmodel(block3)
...
#         loss = self.loss_fn(logits, batch['target'].long(), self.n_classes)
        ...
        if self.training:
            ...
            local_loss = self.local_loss_fn(local_logits, batch['target'].long())
            return {'loss': loss, 'local_loss': local_loss, "target": batch['target'], "preds_conf":preds_conf,'preds_cls':preds_cls}
