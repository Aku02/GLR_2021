...
class Net(nn.Module):
    ...
    def forward(self, batch, scale_depricated=1):
        ...
        x_embs = []
        for scale in [0.7,1,1.4]:
            
            if scale != 1:
                x1 = F.interpolate(x, scale_factor=scale)
            else:
                x1 = x
            x2 = self.backbone(x1)
            ...
            x_embs += [self.neck(x_fused)]
        x_emb = torch.stack(x_embs).mean(0)
