...
class Net(nn.Module):
    def __init__(self, cfg, dataset):
        ...
        self.conv1 = nn.Conv2d(backbone_out, 512, 1, 1)
        self.bn = nn.BatchNorm2d(512)
        self.local_loss_fn = nn.CrossEntropyLoss()
        self.ae_loss_fn = nn.MSELoss()
        self.localmodel = SpatialAttention2d()
        self.att_cls = ResHead(512,self.n_classes)
        self.auto_encoder = nn.Sequential(nn.Conv2d(512, 128, 1, 1),nn.ReLU(),nn.Conv2d(128, 512, 1, 1))
    ...
    def forward(self, batch, scale=1):
        ...
        block3 = x.detach()
        block3 = self.conv1(block3)
        block3 = self.bn(block3)
        att_score = self.localmodel(block3)
        
        block3b = self.auto_encoder(block3)
        feature_map_norm = F.normalize(block3b, p=2, dim=1)
        att = att_score.expand_as(feature_map_norm)
        local_features = att * feature_map_norm
        ...
        if self.cfg.headless:
            r = {"target": batch['target'],'embeddings': x_emb}
            if self.cfg.return_local_features:
                r['local_features'] = local_features
                r['att_score'] = att_score
            return r
        
        local_logits = self.att_cls(local_features)
...
#         loss = self.loss_fn(logits, batch['target'].long(), self.n_classes)
        ...
        if self.training:
            ...
            local_loss1 = self.local_loss_fn(local_logits, batch['target'].long())
            ae_loss = 10 * self.ae_loss_fn(block3,block3b)
            local_loss = local_loss1 + ae_loss
            return {'loss': loss, 'local_loss': local_loss, 'local_loss1': local_loss1,'ae_loss': ae_loss,"target": batch['target'], "preds_conf":preds_conf,'preds_cls':preds_cls}
