...
# #         self.loss_fn = ArcFaceLossAdaptiveMargin(dataset.margins,cfg.arcface_s)
...
class Net(nn.Module):
    ...
    def __init__(self, cfg, ds =None):
        super(Net, self).__init__()
        self.cfg = cfg
        self.enet = geffnet.create_model(cfg.backbone, pretrained=cfg.pretrained)
        self.feat = nn.Linear(self.enet.classifier.in_features, 512)
        self.swish = Swish_module()
        if not self.cfg.headless: 
            self.metric_classify = ArcMarginProduct_subcenter(512, cfg.n_classes)
    ...
    def forward(self, batch):
        x = batch['input']
        x = self.extract(x)
        x_emb = self.swish(self.feat(x))
        if self.cfg.headless:
            return {"target": batch['target'],'embeddings': x_emb}
        
        logits = self.metric_classify(x_emb)
        preds = logits.softmax(1)
        preds_conf, preds_cls = preds.max(1)
        if self.training:
            loss = self.loss_fn(logits, batch['target'].long())
