...
#             backbone_out = self.backbone.feature_info[-1]['num_chs']
    ...
    def forward(self, batch):
        ...
        patches = patches.contiguous().view(patches.size(0), patches.size(1), -1, self.kernel_size, self.kernel_size)
        patches = patches.permute(0,2,1,3,4)
        bs, n_patches, c, w, h = patches.shape
        x = patches.reshape(bs*n_patches,c,w,h)
        
        x = self.backbone(x)

        x_emb = self.neck(x)
        bsp, emb_dim = x_emb.shape
        x_emb = x_emb.reshape(bs,n_patches,emb_dim)
        x_emb = x_emb.mean(1)
